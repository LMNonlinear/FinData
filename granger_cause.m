function [lin_trans_entropy,x_lag,y_lag] = granger_cause(x,y,max_lag)
...
while i <= max_lag
    ...
    xstar = [ones(T-(max(i,x_lag)),1) zeros(T-(max(i,x_lag)),x_lag+i)];
...
lin_trans_entropy = 0.5*log(RSS_R(x_lag,:)/RSS_U(y_lag,:));
